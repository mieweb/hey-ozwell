...
ev = WakeWordEvaluator("../prod/js/models/hey-ozwell.onnx","eval/pretrained")
...
ds=load_dataset("MLCommons/peoples_speech","clean",split="test",streaming=True,trust_remote_code=True)
for ex in ds:
    ...
    if n%200==0:
        ...
        np.save("heybuddy/precalculated/mined_false_fires_hey.npy", np.concatenate([x for x in fires if len(x)]) if nf else np.zeros((0,EMB_FRAMES,EMB_DIM),"float32"))
        np.save("heybuddy/precalculated/mined_near_misses_hey.npy", np.concatenate([x for x in nears if len(x)]) if nn else np.zeros((0,EMB_FRAMES,EMB_DIM),"float32"))
    if n>=20000: break
