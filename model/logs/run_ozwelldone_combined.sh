...
RES=logs/FINAL_RESULTS_COMBINED.txt
ACC="$P/eleven_combined_pos_done.npy"
ONNX="checkpoints/scratch-onnx/ozwelldone_combined.onnx"
PRODDONE="../prod/js/models/ozwell-i'm-done.onnx"
echo "=== ozwell-done COMBINED diverse retrain (EL accents + cross-lingual + VD) $(date) ===" > "$RES"
...
echo "" | tee -a "$RES"; echo "=== COMBINED DONE $(date) — adopt only if cross-vendor UP, originals HELD, FP HELD ===" | tee -a "$RES"
