...
ds=load_dataset("MLCommons/peoples_speech","clean",split="train",streaming=True,trust_remote_code=True)
for ex in ds:
    ...
    if n%200==0:
        ...
        np.save("heybuddy/precalculated/mined_false_fires.npy", np.concatenate([x for x in fires if len(x)]) if nf else np.zeros((0,EMB_FRAMES,EMB_DIM),"float32"))
        np.save("heybuddy/precalculated/mined_near_misses.npy", np.concatenate([x for x in nears if len(x)]) if nn else np.zeros((0,EMB_FRAMES,EMB_DIM),"float32"))
    if n>=20000: break
