# Does the per-window recall cost survive at the UTTERANCE level? Pass-1 fires several windows per real wake;
# only ONE needs to clear the veto for detection. Group your recording's firing windows into utterances
# (contiguous firing regions), report per-UTTERANCE survival vs per-WINDOW. Verifier trained on diverse
# synthetic positives (fresh eleven train voices) + mined real false-fires.
...
# train positives = fresh eleven train voices (the config that gives 98-99% on held-out voices)
...
# your recording: ALL windows in time order + pass-1 scores
W,S=winsc(load_16k_mono("../../real_audio/Oz-done.wav"))
Pv=m.predict_proba(flat(W))[:,1]
fire=S>=0.5
# group consecutive firing windows into utterances (gap > 5 windows = new utterance)
utts=[]; cur=[]
for i,f in enumerate(fire):
    if f: cur.append(i)
    elif cur and (i-cur[-1])>5: utts.append(cur); cur=[]
if cur: utts.append(cur)
print(f"firing windows: {int(fire.sum())} | grouped into {len(utts)} utterances\n")
print("   t    per-WINDOW kept   per-UTTERANCE kept (any window survives)")
for t in [0.05,0.1,0.2,0.5]:
    win_keep=(Pv[fire]>=t).mean()*100
    utt_keep=np.mean([ (Pv[u]>=t).any() for u in utts ])*100
    print(f"  {t:.2f}      {win_keep:4.0f}%             {utt_keep:4.0f}%   ({sum((Pv[u]>=t).any() for u in utts)}/{len(utts)} utterances)")
print("\nmax verifier P per utterance:", " ".join(f"{max(Pv[u]):.2f}" for u in utts))
print("UTT_DONE")
